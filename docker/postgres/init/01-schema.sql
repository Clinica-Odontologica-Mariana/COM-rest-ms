...
ALTER TABLE service_price
    ADD CONSTRAINT ex_service_price_period
    EXCLUDE USING gist (
        service_id WITH =,
...
ALTER TABLE service_cost
    ADD CONSTRAINT ex_service_cost_period
    EXCLUDE USING gist (
        service_id WITH =,
        cost_type_id WITH =,
        daterange(valid_from, COALESCE(valid_to, 'infinity'::DATE), '[]') WITH &&
...
ALTER TABLE working_hours
    ADD CONSTRAINT ex_working_hours_overlap
    EXCLUDE USING gist (
        clinic_id WITH =,
        day_of_week WITH =,
        tsrange(
...
ALTER TABLE appointment
    ADD CONSTRAINT ex_appointment_professional_overlap
    EXCLUDE USING gist (
        clinic_id WITH =,
...
ALTER TABLE appointment
    ADD CONSTRAINT ex_appointment_workplace_overlap
    EXCLUDE USING gist (
        clinic_id WITH =,
        workplace_id WITH =,
        tstzrange(start_datetime, end_datetime, '[)') WITH &&
...
(
    ...
    CONSTRAINT fk_visit_appointment_patient
...
BEGIN
    ...
    IF v_blocks_schedule IS NULL THEN
        RAISE EXCEPTION 'Status de agendamento invalido.';
    END IF;

    NEW.blocks_schedule = v_blocks_schedule;
    ...
    IF v_clinic_timezone IS NULL THEN
        RAISE EXCEPTION 'Clinica invalida.';
    END IF;

    -- Serializa validacoes por clinica/profissional para reduzir corrida entre transacoes.
    PERFORM pg_advisory_xact_lock(hashtext(NEW.clinic_id::TEXT || ':' || NEW.professional_id::TEXT));

    IF NEW.workplace_id IS NOT NULL THEN
        PERFORM pg_advisory_xact_lock(hashtext(NEW.clinic_id::TEXT || ':' || NEW.workplace_id::TEXT));
    END IF;

    IF NEW.blocks_schedule THEN
        v_local_start = NEW.start_datetime AT TIME ZONE v_clinic_timezone;
        v_local_end = NEW.end_datetime AT TIME ZONE v_clinic_timezone;
...
CREATE OR REPLACE FUNCTION fn_validate_clinical_visit()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    IF NEW.appointment_id IS NOT NULL THEN
        ...
        IF v_appointment_professional IS NULL THEN
            RAISE EXCEPTION 'Agendamento invalido para o paciente informado.';
        END IF;

        IF v_appointment_professional <> NEW.professional_id THEN
...
CREATE OR REPLACE FUNCTION fn_validate_treatment_plan_item()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    IF NEW.completed_visit_id IS NOT NULL THEN
        SELECT patient_id
          INTO v_visit_patient
          FROM clinical_visit
         WHERE id = NEW.completed_visit_id;

        IF v_visit_patient IS NULL THEN
...
AS $$
BEGIN
    IF NEW.net_amount <> NEW.gross_amount - NEW.discount_amount THEN
...
BEGIN
    ...
    IF v_invoice_patient IS NULL THEN
        RAISE EXCEPTION 'Cobranca invalida.';
    END IF;

    IF v_invoice_patient <> NEW.patient_id THEN
...
CREATE TRIGGER trg_clinical_visit_updated_at
    BEFORE UPDATE ON clinical_visit
    FOR EACH ROW EXECUTE FUNCTION fn_set_updated_at();
...
CREATE TRIGGER trg_insurance_provider_updated_at
    BEFORE UPDATE ON insurance_provider
    FOR EACH ROW EXECUTE FUNCTION fn_set_updated_at();
...
CREATE TRIGGER trg_validate_clinical_visit
    BEFORE INSERT OR UPDATE ON clinical_visit
    FOR EACH ROW EXECUTE FUNCTION fn_validate_clinical_visit();

CREATE TRIGGER trg_validate_treatment_plan_item
    BEFORE INSERT OR UPDATE ON treatment_plan_item
    FOR EACH ROW EXECUTE FUNCTION fn_validate_treatment_plan_item();
...
CREATE TRIGGER trg_audit_medical_record_note
    AFTER INSERT OR UPDATE OR DELETE ON medical_record_note
    FOR EACH ROW EXECUTE FUNCTION fn_audit_row();

CREATE TRIGGER trg_audit_medical_record_attachment
    AFTER INSERT OR UPDATE OR DELETE ON medical_record_attachment
    FOR EACH ROW EXECUTE FUNCTION fn_audit_row();
...
CREATE TRIGGER trg_audit_clinical_visit
    AFTER INSERT OR UPDATE OR DELETE ON clinical_visit
    FOR EACH ROW EXECUTE FUNCTION fn_audit_row();

CREATE TRIGGER trg_audit_treatment_plan
    AFTER INSERT OR UPDATE OR DELETE ON treatment_plan
    FOR EACH ROW EXECUTE FUNCTION fn_audit_row();

CREATE TRIGGER trg_audit_treatment_plan_item
    AFTER INSERT OR UPDATE OR DELETE ON treatment_plan_item
    FOR EACH ROW EXECUTE FUNCTION fn_audit_row();
...
CREATE TRIGGER trg_audit_patient_consent
    AFTER INSERT OR UPDATE OR DELETE ON patient_consent
    FOR EACH ROW EXECUTE FUNCTION fn_audit_row();
...
CREATE INDEX idx_appointment_external_calendar
    ON appointment (calendar_provider_id, external_calendar_event_id)
...
INSERT INTO clinical_visit_status (code, name) VALUES
    ('OPEN', 'Aberto'),
...
INSERT INTO treatment_plan_status (code, name) VALUES
    ('DRAFT', 'Rascunho'),
...
INSERT INTO tooth_condition (code, name) VALUES
    ('HEALTHY', 'Saudavel'),
    ('CARIES', 'Carie'),
    ('RESTORED', 'Restaurado'),
    ('MISSING', 'Ausente'),
...
INSERT INTO tooth_surface (code, name) VALUES
    ('M', 'Mesial'),
    ('D', 'Distal'),
    ('V', 'Vestibular'),
    ('L', 'Lingual'),
...
INSERT INTO calendar_provider (code, name) VALUES
    ('GOOGLE', 'Google Agenda'),
    ('OUTLOOK', 'Microsoft Outlook'),
    ('MANUAL', 'Manual')
ON CONFLICT (code) DO NOTHING;

INSERT INTO calendar_sync_status (code, name) VALUES
    ('PENDING', 'Pendente'),
    ('SYNCED', 'Sincronizado'),
    ('FAILED', 'Falhou'),
...
INSERT INTO consent_type (code, name, description) VALUES
    ('DATA_PROCESSING', 'Tratamento de dados pessoais', 'Consentimento para tratamento de dados pessoais conforme LGPD.'),
    ('IMAGE_STORAGE', 'Armazenamento de imagens e exames', 'Consentimento para armazenamento de imagens, exames e anexos clinicos.'),
    ('MARKETING', 'Comunicacoes de marketing', 'Consentimento para envio de comunicacoes promocionais.'),
    ('TREATMENT', 'Tratamento odontologico', 'Consentimento informado para realizacao de tratamento odontologico.')
ON CONFLICT (code) DO NOTHING;
...
INSERT INTO service_cost_type (code, name) VALUES
    ('MATERIAL', 'Material'),
...
INSERT INTO social_platform (code, name) VALUES
    ('INSTAGRAM', 'Instagram'),
    ('FACEBOOK', 'Facebook'),
    ('WHATSAPP', 'WhatsApp'),
    ('TIKTOK', 'TikTok'),
...
INSERT INTO specialty (code, name) VALUES
    ('GENERAL_DENTISTRY', 'Clinico geral'),
    ('ORTHODONTICS', 'Ortodontia'),
    ('ENDODONTICS', 'Endodontia'),
    ('PERIODONTICS', 'Periodontia'),
    ...
    ('AESTHETIC_DENTISTRY', 'Odontologia estetica')
ON CONFLICT (code) DO NOTHING;
