...
public class SocialLinkService {
	...
	public SocialLinkService(SocialLinkRepository socialLinkRepository, ClinicRepository clinicRepository,
	...
	public SocialLinkDto create(SocialLinkCreateDto request) {
		...
		SocialLinkModel model = SocialLinkModel.create(request.clinicId(), request.platformId(), request.url());
	...
	public List<SocialLinkDto> findByClinicId(UUID clinicId) {
		return socialLinkRepository.findAllByClinicIdOrderByCreatedAtAsc(clinicId).stream().map(this::toModel)
				.map(this::toDto).toList();
	}

	@Transactional(readOnly = true)
	public SocialLinkDto findById(UUID id) {
		SocialLinkEntity entity = socialLinkRepository.findById(id).orElseThrow(
				() -> new AppException(HttpStatus.NOT_FOUND, "SOCIAL_LINK_NOT_FOUND", "Social link not found"));
	...
	public SocialLinkDto update(UUID id, SocialLinkUpdateDto request) {
		...
		if (socialLinkRepository.existsByClinicIdAndPlatformIdAndIdNot(entity.getClinicId(), request.platformId(),
				id)) {
			throw new AppException(HttpStatus.CONFLICT, "SOCIAL_LINK_PLATFORM_CONFLICT",
					"A social link for this clinic and platform already exists");
		}

		SocialLinkModel model = new SocialLinkModel(id, entity.getClinicId(), request.platformId(), request.url());
	...
	private SocialLinkModel toModel(SocialLinkEntity entity) {
		return new SocialLinkModel(entity.getId(), entity.getClinicId(), entity.getPlatformId(), entity.getUrl());
	}

	private SocialLinkDto toDto(SocialLinkModel model) {
		return new SocialLinkDto(model.id(), model.clinicId(), model.platformId(), model.url());
	}
}
