...
@DisplayName("Social link parameterized")
class SocialLinkControllerParameterizedTest {
	...
	void shouldRejectInvalidCreatePayloads(String scenario, String payload) throws Exception {
		mockMvc.perform(post("/api/v1/social-links").contextPath(CONTEXT_PATH)
	...
	static Stream<Arguments> invalidCreatePayloads() {
		return Stream.of(Arguments.of("missing clinicId", """
				...
				""".formatted(PLATFORM_ID)), Arguments.of("missing platformId", """
				{
				  "clinicId": "%s",
				  "url": "https://instagram.com/clinica"
				}
				""".formatted(CLINIC_ID)), Arguments.of("missing url", """
				{
				  "clinicId": "%s",
				  "platformId": "%s"
				}
				""".formatted(CLINIC_ID, PLATFORM_ID)), Arguments.of("blank url", """
				{
				  "clinicId": "%s",
				  "platformId": "%s",
				  "url": "   "
				}
				""".formatted(CLINIC_ID, PLATFORM_ID)), Arguments.of("url without http/https scheme", """
				{
				  "clinicId": "%s",
				  "platformId": "%s",
				  "url": "instagram.com/clinica"
				}
				""".formatted(CLINIC_ID, PLATFORM_ID)), Arguments.of("url with ftp scheme", """
				{
				  "clinicId": "%s",
				  "platformId": "%s",
				  "url": "ftp://instagram.com/clinica"
				}
				""".formatted(CLINIC_ID, PLATFORM_ID)));
	}
}
